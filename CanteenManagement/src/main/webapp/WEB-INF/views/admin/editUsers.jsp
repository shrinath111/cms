...
function alertInfo()
{
	swal({
		  title: "User Deleted!",
		  text: "You've successfully deleted an User !",
		  icon: "success",
...
<body>
	...
	<section>
		...
		<div class="tbl-content">
			<table cellpadding="0" cellspacing="0" border="0">
				<tbody>
					<c:forEach var="v" items="${requestScope.usersss}">
					<tr>
						<td>${v.userId}</td>
						<td>${v.userName}</td>
						<td>${v.password}</td>
						<td>${v.phoneNum}</td>
						<td><a
					href="<spring:url value='/admin/deletepage?userId=${v.userId}'/>" onclick="alertInfo()" >Delete</a></td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
